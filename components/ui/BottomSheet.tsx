...
type SnapPointKey = 'CLOSED' | 'HALF' | 'FULL' | string;
type CloseButtonPosition = 'left' | 'right';
...
interface BottomSheetProps {
	...
	onSnapPointChange?: (snapPoint: SnapPointKey) => void;
	modal?: boolean;
	zIndex?: number;
}
...
const BottomSheet: React.FC<BottomSheetProps> = ({
	...
	const calculatedSnapPoints = snapPoints || DEFAULT_SNAP_POINTS;

	// Get the initial snap point value
	const initialPosition =
		calculatedSnapPoints[initialSnapPoint] || calculatedSnapPoints.HALF;
	...
	const [currentSnapPoint, setCurrentSnapPoint] =
		useState<SnapPointKey>(initialSnapPoint);

	// Ref for tracking the current position
	const currentPosition = useRef<number>(initialPosition);
	...
	const panResponder = useRef(
		PanResponder.create({
			...
			onPanResponderRelease: (_, gestureState: PanResponderGestureState) => {
				...
				let nextSnapPoint: SnapPointKey = 'CLOSED';

				// Sort snap points in descending order
				const sortedSnapPoints = Object.entries(calculatedSnapPoints).sort(
					(a, b) => b[1] - a[1]
				);
	...
	const snapToPoint = (snapPoint: SnapPointKey) => {
